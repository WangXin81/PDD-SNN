...
try:
    ...
    from spikingjelly.activation_based import neuron_kernel, cu_kernel_opt
...
class NegBaseNode(base.MemoryModule):
    def __init__(self, v_threshold: float = 1., neg_v_threshold: float = 1., v_reset: float = None,
                 surrogate_function: Callable = surrogate.Sigmoid(alpha=4.0), detach_reset: bool = False):
        """
    ...
    def neuronal_reset(self, spike):
        ...
        * :ref:`中文API <BaseNode.neuronal_reset-cn>`
        ...
        if self.v_reset is None:
            # 软重置: 正脉冲减阈值，负脉冲加阈值(即减去负的阈值贡献)
            # 公式: v = v - (pos * v_th - neg * neg_v_th)
            delta_v = pos_spike.float() * self.v_threshold - neg_spike.float() * self.neg_v_threshold
...
#             self.v = self.v - spike_d_vth
    ...
    def extra_repr(self):
        return f'v_threshold={self.v_threshold}, neg_v_threshold={self.neg_v_threshold},v_reset={self.v_reset}, detach_reset={self.detach_reset}'
...
class NegIFNode(NegBaseNode):
    ...
    def forward(self, x: torch.Tensor):
        if self.cupy_fp32_inference and cupy is not None and not self.training and x.dtype == torch.float32:
            ...
            code += r'''
                {
                    ...
                    if (index < numel)
                    {
                        ...
                        float delta_v = pos_spike * v_threshold - neg_spike * neg_v_threshold;
            ...
            code += rf'''
                        // [修改这里] 统一使用减法: v -= delta_v
                        // 正脉冲: v -= v_th (正确)
                        // 负脉冲: v -= (-neg_v_th) -> v += neg_v_th (正确，回拉电压)
                        {' v[index] = (1.0f - abs(spike[index])) * v[index] + spike[index] * v_reset;' if hard_reset else 'v[index] -= delta_v;'}
            '''
...
class MultiStepNegIFNode(NegIFNode):
    def __init__(self, v_threshold: float = 0.5, neg_v_threshold: float = 0.5, v_reset: float = None,
                 momentum: float = 0.1,
                 surrogate_function: Callable = surrogate.Sigmoid(alpha=4.0), detach_reset: bool = False, backend='torch',
                 lava_s_cale=1 << 6):
        """
    ...
    def forward(self, x_seq: torch.Tensor):
        ...
        if self.backend == 'torch':
            ...
            spike_seq, self.v_seq = neuron_kernel.MultiStepNegIFNodePTT.apply(
                x_seq.flatten(1), self.v.flatten(0), self.v_threshold, self.neg_v_threshold, self.v_reset,
