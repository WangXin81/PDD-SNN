...
from .wavelet import Haar2DForward, Haar2DInverse
...
class FusionModule(nn.Module):
    def __init__(self, time_steps=5, base_ch=64, v_th=0.1, v_reset=0.0, tau=2.0, spike_type='binary', soft_reset=False, ablation_mode='dual'):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.ablation_mode == 'local_only':
            # 模式 A: 仅保留空域 (Local)，屏蔽频域 (Global)
            freq_feat = freq_feat * 0.0
            
        elif self.ablation_mode == 'global_only':
            # 模式 B: 仅保留频域 (Global)，屏蔽空域 (Local)
            spatial_feat = spatial_feat * 0.0
...
class StackedFusionReconstructionNet(nn.Module):
    def __init__(self, num_fusion_modules=8, time_steps=5, base_ch=64,
                 upscale_factor=2, v_th=0.1, v_reset=0.0, tau=2.0, spike_type='binary', soft_reset=False, ablation_mode='dual'):
        super().__init__()
        ...
        self.fusion_modules = nn.ModuleList([
            FusionModule(time_steps, base_ch, v_th, v_reset, tau, spike_type=spike_type, soft_reset=soft_reset, ablation_mode=ablation_mode) for _ in range(num_fusion_modules)
        ])
