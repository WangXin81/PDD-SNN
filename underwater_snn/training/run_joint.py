...
from ..models.networks import DegradationModule, DiscriminatorModule, ReconstructionModule
from ..losses import calculate_psnr, calculate_ssim
from ..data.datasets import create_data_loaders, PairedReferenceHRDataset, UnpairedUnalignedDataset
from ..utils.common import set_seed, ParameterMonitor, create_niqe_prior_if_needed, calculate_distribution_metrics, \
    save_metrics_to_csv, batch_calculate_metrics_degradation, \
    batch_calculate_metrics_reconstruction, EnergyMeter, calculate_uciqe
# from trainer_loss_gan import JointTrainingSystem
from .joint_trainer import JointTrainingSystem
...
#             base_size = getattr(config, 'LR_PATCH_SIZE', 256)
            
#             # 计算 HR 应该是多少（LR * Factor）
#             hr_crop_size = base_size * factor
            
            new_w = w - (w % factor)
            new_h = h - (h % factor)
            # 请修改为：
            # new_w = w - (w % hr_crop_size)
            # new_h = h - (h % hr_crop_size)
...
def prepare_environment():
    ...
    niqe_prior_path = getattr(config, "NIQE_PRIOR_PATH", "") or os.path.join(config.SAVE_DIR, "niqe_water_params.mat")
    if getattr(config, "LR_DIR", ""):
        print("=" * 50)
        print("检查NIQE先验文件...")
        niqe_prior_path = create_niqe_prior_if_needed(config.LR_DIR, niqe_prior_path)
        print("=" * 50)
    return niqe_prior_path

def main():
    NIQE_PRIOR_PATH = prepare_environment()
    # # --- 插入在此处 ---
    # import torch
    # torch.autograd.set_detect_anomaly(True) 
    # print(">>> 已开启自动求导异常检测 (Anomaly Detection ON)")
    # -----------------
    # 创建数据加载器
    # print("创建数据加载器...")
    ...
    if hasattr(config, 'VAL_LR_DIR') and config.VAL_LR_DIR is not None and os.path.exists(config.VAL_LR_DIR):
        ...
        val_dataset = PairedReferenceHRDataset(
            ...
            patch_size=None
            # patch_size=getattr(config, 'LR_PATCH_SIZE', 256)
        )
    else:
        print(f"验证模式: 非配对数据 (Unpaired) - 将使用 Bicubic 模拟退化")
        from ..data.datasets import UnpairedUnalignedDataset
...
#     # ================================================================
    ...
    try:
        from ..evaluation.metrics import ImageQualityEvaluator as DegradationImageQualityEvaluator
        ...
        print("警告: 无法导入退化评估模块，退化模块将跳过质量评估")

    try:
        from ..evaluation.distribution import LRImageDistributionComparator
        distribution_comparator = LRImageDistributionComparator()
        print("分布比较器已初始化")
    except ImportError:
        print("警告: 无法导入 LR 分布比较模块，将跳过分布距离计算")
    ...
    csv_headers = [
        ...
        'val_niqe', 'val_brisque', 'val_uiqm', 'val_uciqe',
...
#                 hr_clamped = torch.clamp(hr, 0, 1)
                ...
                # === [新增对齐逻辑：解决 358 vs 359 这种 1 像素偏差] ===
                # 获取模型输出的实际尺寸 (H, W)
                h_sr, w_sr = hr_reconstructed.shape[-2:]
                h_hr, w_hr = hr.shape[-2:]

                # 如果尺寸不一致，说明 HR 不是倍数，被退化模块切边了
                # 我们对 HR 进行切片，使其与 SR 尺寸完全对齐
                if h_sr != h_hr or w_sr != w_hr:
                    hr = hr[:, :, :h_sr, :w_sr] 
                # ===================================================
                ...
                for b_idx in range(batch_val_size):
                    ...
                    uciqe_score = calculate_uciqe(recon_clamped[b_idx])
                    val_uciqe_cumulative += uciqe_score
...
#                         save_image(comparison, save_path)
                    ...
                    # ==================== [在此处插入显存清理代码] ====================
                    # 1. 手动删除不再需要的大张量
                    del hr_reconstructed, val_lr_lab, recon_clamped, hr_clamped

                    # 2. 每处理 2 张图，强制清理一次显存碎片
                    if batch_idx % 2 == 0: 
        ...
        if val_metric_count > 0:
            ...
            val_uciqe = val_uciqe_cumulative / total_val_images
        ...
        if reconstruction_evaluator:
            print(f"  - NIQE: {val_niqe:.4f} | UIQM: {val_uiqm:.4f} | UCIQE: {val_uciqe:.4f}")
        print("-" * 30)
        ...
        if val_uiqm > best_uiqm:
            ...
            print(f"新最佳 UIQM! 模型已保存: {save_path}")
