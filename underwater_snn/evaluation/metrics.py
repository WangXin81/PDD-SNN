...
def niqe(inputImgData):
    ...
    params = scipy.io.loadmat(join(module_path, '..', 'assets', 'niqe_image_params.mat'))
    pop_mu = np.ravel(params["pop_mu"])
    pop_cov = params["pop_cov"]
...
def batch_niqe_complete(imgs, prior_path=None):
    try:
        scores = [calculate_niqe_complete(img) for img in imgs]
        return float(np.mean(scores))
    except Exception:
...
def batch_niqe_complete_scores(imgs, prior_path=None):
    try:
        return [calculate_niqe_complete(img) for img in imgs]
    except Exception:
        return []
...
# ========= 评估器类接口 =========
class ImageQualityEvaluator:
    def __init__(self, device='cuda', prior_path=None):
        self.device = device
    ...
    def calculate_batch_niqe_scores(self, imgs): return batch_niqe_complete_scores(imgs, self.prior_path)
    def calculate_batch_brisque_scores(self, imgs): return batch_brisque_scores(imgs, self.device)
    def calculate_batch_uiqm_scores(self, imgs): return batch_uiqm_scores(imgs)
