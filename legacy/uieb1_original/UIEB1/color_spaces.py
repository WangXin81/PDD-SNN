...
class RGB2Lab(nn.Module):
    ...
    def _f(self, t):
        delta = 6 / 29
        # 【关键修复 1】: AMP 下 1e-8 会变成 0，导致 pow(1/3) 梯度无穷大。
        # 必须提高到 1e-4 以适应 float16。
        safe_t = torch.clamp(t, min=1e-4) 
        
        # 【关键修复 2】: 显式计算幂运算分支。
        # PyTorch 的 torch.where 会同时计算两个分支的梯度。
        # 如果不先 clamp，即便 t 实际上走了线性分支，幂运算分支在 t=0 处的无穷梯度依然会传回来。
        return torch.where(t > delta ** 3, 
                           safe_t.pow(1 / 3), 
                           (t / (3 * delta ** 2)) + (4 / 29))
    ...
    def forward(self, img):
        B, C, H, W = img.shape
        # 0. 预处理：防止输入出现 0 或负数 (针对水下图像的极端黑区)
        img = torch.clamp(img, min=1e-4, max=1.0)
...
class Lab2RGB(nn.Module):
    ...
    def forward(self, lab_norm):
        ...
        # 【关键修复 3】: 同样适用于反向转换，提高阈值到 1e-4
        safe_linear = torch.clamp(rgb_linear, min=1e-4)
