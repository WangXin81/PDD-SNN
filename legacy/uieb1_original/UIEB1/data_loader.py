...
class PairedReferenceHRDataset(Dataset):
    """
    [修正版] 配对数据集加载器 - 支持 patch_size 随机裁剪
    """
    def __init__(self, hr_dir, lr_dir, upscale_factor=4, transform=None, patch_size=None):
        super().__init__()
        self.hr_dir = hr_dir
        self.lr_dir = lr_dir
        self.upscale_factor = upscale_factor
        self.transform = transform
        self.patch_size = patch_size  # [新增] 接收裁剪尺寸参数
        ...
        if self.lr_dir and os.path.exists(self.lr_dir):
            self.lr_filenames = sorted([x for x in os.listdir(lr_dir) if is_image_file(x)])
            if len(self.hr_filenames) != len(self.lr_filenames):
                print(f"⚠️ 警告: HR数量 ({len(self.hr_filenames)}) 与 LR数量 ({len(self.lr_filenames)}) 不一致！")
        else:
            self.lr_filenames = []
            print(f"⚠️ 警告: 未找到有效的 LR 目录: {self.lr_dir}")
    ...
    def __getitem__(self, index):
        ...
        # ==================== [新增] 随机裁剪逻辑 ====================
        if self.patch_size is not None and lr_img is not None:
            ...
            if w_lr >= self.patch_size and h_lr >= self.patch_size:
                # A. 随机生成 LR 的裁剪坐标
                x1 = random.randint(0, w_lr - self.patch_size)
                y1 = random.randint(0, h_lr - self.patch_size)
                
                # B. 裁剪 LR
                lr_img = lr_img.crop((x1, y1, x1 + self.patch_size, y1 + self.patch_size))
                
                # C. 计算对应的 HR 裁剪坐标 (坐标 * 放大倍数)
                x1_hr = x1 * self.upscale_factor
                y1_hr = y1 * self.upscale_factor
                hr_crop_size = self.patch_size * self.upscale_factor
                
                # D. 裁剪 HR
                # 注意：这里加一个简单的边界保护，防止计算误差导致越界
                if x1_hr + hr_crop_size <= w_hr and y1_hr + hr_crop_size <= h_hr:
                    hr_img = hr_img.crop((x1_hr, y1_hr, x1_hr + hr_crop_size, y1_hr + hr_crop_size))
                else:
                    # 如果 HR 尺寸计算有微小误差，强制 resize 到标准大小
                    hr_img = hr_img.resize((hr_crop_size, hr_crop_size), Image.BICUBIC)
...
class UnpairedUnalignedDataset(Dataset):
    """
    修改版：支持训练时随机裁剪 Patch，验证时保持原图尺寸
    """
    def __init__(self, hr_data, lr_data, hr_transform=None, lr_transform=None, is_train=True, upscale_factor=4, patch_size=None):
        super().__init__()
        self.hr_images = hr_data
        self.lr_images = lr_data
        self.hr_transform = hr_transform
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train and self.patch_size is not None:
            ...
            if w_lr >= self.patch_size and h_lr >= self.patch_size:
                ...
                lr_img = lr_img.crop((x1, y1, x1 + self.patch_size, y1 + self.patch_size))
                # 裁剪 HR (对应坐标 * 倍率)
                x1_hr, y1_hr = x1 * self.upscale_factor, y1 * self.upscale_factor
                p_hr = self.patch_size * self.upscale_factor
                hr_img = hr_img.crop((x1_hr, y1_hr, x1_hr + p_hr, y1_hr + p_hr))

        # 4. 应用 Transform (ToTensor 等)
        if self.hr_transform:
            hr_img = self.hr_transform(hr_img)
        if self.lr_transform:
            lr_img = self.lr_transform(lr_img)
...
def create_data_loaders(config):
    ...
    train_dataset = UnpairedUnalignedDataset(
        ...
        lr_transform=base_transform,
        is_train=True,
        patch_size=config.LR_PATCH_SIZE,  # [关键] 训练用 Patch
        upscale_factor=getattr(config, 'UPSCALE_FACTOR', 4)
    )

    # 2. 创建验证集：不传 patch_size (默认为 None)，使用 480x640 全尺寸
    val_dataset = UnpairedUnalignedDataset(
        hr_data=val_hr_files,
        lr_data=val_lr_files,
        hr_transform=base_transform,   
