...
import random
from config_BS import config
import csv
from color_spaces import (RGB2Lab, Lab2RGB)
from models import DegradationModule, DiscriminatorModule, ReconstructionModule, ReconstructionDiscriminator
from losses1 import VGGPerceptualLoss, LowFrequencyLoss, MultiBranchWaterLoss, EdgeLoss, ct_loss, mmd_rbf, \
    calculate_psnr, calculate_ssim, DINOv2PerceptualLoss, GradientLoss, FocalFrequencyLoss, SRVGGPerceptualLoss
from utils import GeometricAugmentation, ParameterMonitor
from data_loader import UnpairedUnalignedDataset, PairedReferenceHRDataset
...
#                 self.optimizer_d_hr.load_state_dict(checkpoint['optimizer_d_hr_state_dict'])
    ...
    def train_degradation_step(self, hr_rgb, real_lr_rgb):
        ...
        if self.degrade_accum_count == self.accumulation_steps:
            if config.USE_AMP:
                ...
                torch.nn.utils.clip_grad_norm_(self.degradation_module.parameters(), max_norm=0.01)
    ...
    def train_step(self, hr_imgs, lr_real_imgs):
        """
        联合训练步骤 (修正版 - 已对齐优化器、Loss和Scaler名称)
        """
        from config_BS import config
        ...
        # 生成假 LR 用于判别器 (Baseline 或 Ours 模式)
        with torch.no_grad():
            if config.TRAIN_BASELINE:
                # Baseline 模式：用 Bicubic 造假 LR
                lr_bicubic = F.interpolate(hr_imgs, scale_factor=1/config.UPSCALE_FACTOR, mode='bicubic', align_corners=False)
                lr_bicubic = torch.clamp(lr_bicubic, 0, 1)
                generated_lab_lr = self.rgb2lab(lr_bicubic)
            else:
                # Ours 模式：用退化模块造假 LR
                self._reset_all_neurons()
                generated_lab_lr = self.degradation_module(hr_imgs)
        
        # 注意：这里如果需要训练判别器，逻辑应与 train_degradation_step 类似
        # 鉴于你目前是 Baseline 调试，暂且跳过判别器 Backward 以免引入更多变量错误

        # ------------------------------------------------------------------

        # 2. 生成器训练 (退化模块 + 重建模块)
        # 修正：使用构造函数中定义的名称 optimizer_g_reconstruct
        self.optimizer_g_reconstruct.zero_grad()
        if not config.TRAIN_BASELINE:
            self.optimizer_g_degrade.zero_grad()
            
        self._reset_all_neurons() # 重置 SNN 状态

        # === 数据源切换 ===
        if config.TRAIN_BASELINE:
            with torch.no_grad():
                lr_down = F.interpolate(hr_imgs, scale_factor=1/config.UPSCALE_FACTOR, mode='bicubic', align_corners=False)
                lr_down = torch.clamp(lr_down, 0, 1)
                reconstruction_input = self.rgb2lab(lr_down)
            
            total_degradation_loss = torch.tensor(0.0, device=self.device)
            loss_ct = torch.tensor(0.0, device=self.device)
            generated_lab_lr = reconstruction_input
        else:
            generated_lab_lr = self.degradation_module(hr_imgs)
            reconstruction_input = generated_lab_lr
            # 此处应调用退化损失计算，暂设为0或保留你原有的逻辑
            loss_ct = torch.tensor(0.0, device=self.device) 
            total_degradation_loss = torch.tensor(0.0, device=self.device) 
        ...
        with torch.cuda.amp.autocast(enabled=config.USE_AMP):
            ...
            # 4. 计算重建损失
            # 修正：使用 __init__ 中定义的 l1_loss 和 recon_vgg_loss (或 vgg_perceptual_loss)
            reconstruction_l1 = self.l1_loss(hr_reconstructed, hr_imgs)
            
            # 建议使用 SRVGGPerceptualLoss (recon_vgg_loss)
            perceptual_loss = self.recon_vgg_loss(hr_reconstructed, torch.clamp(hr_imgs, 0, 1))
            
            # 梯度损失
            grad_loss = self.gradient_loss_fn(hr_reconstructed, hr_imgs)

            # 5. 总损失汇总
            if config.TRAIN_BASELINE:
                final_loss = reconstruction_l1 + 0.1 * perceptual_loss + 0.5 * grad_loss
            else:
                final_loss = (reconstruction_l1 + 0.1 * perceptual_loss + 0.5 * grad_loss) + total_degradation_loss

        # 6. Backward 与 优化
        # 修正：使用 __init__ 中定义的 scaler_g_reconstruct
        if config.USE_AMP and self.scaler_g_reconstruct is not None:
            self.scaler_g_reconstruct.scale(final_loss).backward()
        ...
        # 返回日志
        return {
            'reconstruction_loss': reconstruction_l1.item(),
            'degradation_loss': total_degradation_loss.item(),
            'g_loss': final_loss.item(),
            'loss_ct': loss_ct.item(),
            'hr_reconstructed': hr_reconstructed.detach(),
            'generated_lab_lr': generated_lab_lr.detach(),
            'perceptual_loss_reconstruct': perceptual_loss.item(),
            'gradient_loss': grad_loss.item(),
            'reconstruction_total_loss': final_loss.item()
        }
