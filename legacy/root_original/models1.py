...
#     def __init__(self, in_channels=1, base_channels=32, time_steps=8, spike_type='binary', use_temporal=True):
#         super().__init__()
...
#                 self.lif1 = MultiStepPmLIFNode(tau=1.2, v_th=0.5, surrogate_function=surrogate.Sigmoid())
#                 self.lif2 = MultiStepPmLIFNode(tau=1.2, v_th=0.5, surrogate_function=surrogate.Sigmoid())
#                 self.lif3 = MultiStepPmLIFNode(tau=1.2, v_th=0.5, surrogate_function=surrogate.Sigmoid())
#             else:
#                 self.lif1 = MultiStepTernaryPmLIFNode(tau=1.2, v_th=0.5, surrogate_function=surrogate.Sigmoid())
#                 self.lif2 = MultiStepTernaryPmLIFNode(tau=1.2, v_th=0.5, surrogate_function=surrogate.Sigmoid())
#                 self.lif3 = MultiStepTernaryPmLIFNode(tau=1.2, v_th=0.5, surrogate_function=surrogate.Sigmoid())
...
#         super().__init__()
#         self.device = device
#         from neurons1 import TemporalLABEnhancer
...
# #     def forward(self, lr_rgb, return_features=False):
# #         functional.reset_net(self)
# #         # self._safe_reset()
# #         if lr_rgb.shape[1] == 3:
# #             l_channel = lr_rgb[:, 0:1, :, :]
# #         else:
# #             l_channel = lr_rgb

# #         if return_features:
# #             return self.discriminator(l_channel, return_features=True)
# #         else:
# #             return self.discriminator(l_channel)
...
#         # 添加输入验证
#         self.expected_channels = 1  # 期望L通道输入
        
#         self.discriminator = TemporalSNNDiscriminatorLocal(
#             in_channels=1,  # 单通道L输入
#             base_channels=32,
#             time_steps=degradation_config['time_steps'],
#             spike_type=degradation_config['spike_type'],
#             use_temporal=degradation_config['use_temporal']
...
#             raise ValueError(f"判别器期望{self.expected_channels}通道输入(L通道)，但得到{input.shape[1]}通道")
...
#                  v_th=0.5, v_reset=0.0, tau=1.2, device='cuda'):
#         super().__init__()
#         self.device = device
...
class TemporalSNNDiscriminatorLocal(nn.Module):
    def __init__(self, in_channels=3, base_channels=32, time_steps=8, spike_type='binary', use_temporal=True):
        ...
        if self.use_temporal:
            ...
            self.conv3 = nn.Conv2d(base_channels * 2, base_channels * 4, kernel_size=3, stride=2, padding=1)
            if spike_type == 'binary':
                self.lif1 = MultiStepPmLIFNode(tau=2.0, v_th=0.3, surrogate_function=surrogate.Sigmoid(alpha=1.0))
                self.lif2 = MultiStepPmLIFNode(tau=2.0, v_th=0.3, surrogate_function=surrogate.Sigmoid(alpha=1.0))
                self.lif3 = MultiStepPmLIFNode(tau=2.0, v_th=0.3, surrogate_function=surrogate.Sigmoid(alpha=1.0))
            else:
                self.lif1 = MultiStepTernaryPmLIFNode(tau=2.0, v_th=0.3, surrogate_function=surrogate.Sigmoid(alpha=1.0))
                self.lif2 = MultiStepTernaryPmLIFNode(tau=2.0, v_th=0.3, surrogate_function=surrogate.Sigmoid(alpha=1.0))
                self.lif3 = MultiStepTernaryPmLIFNode(tau=2.0, v_th=0.3, surrogate_function=surrogate.Sigmoid(alpha=1.0))
            self.gap = nn.AdaptiveAvgPool2d(1)
        else:
            self.conv1 = nn.Conv2d(in_channels, base_channels, kernel_size=3, stride=2, padding=1)
            self.conv2 = nn.Conv2d(base_channels, base_channels * 2, kernel_size=3, stride=2, padding=1)
...
class ReconstructionModule(nn.Module):
    def __init__(self, num_fusion_modules=3, time_steps=5, base_ch=64, upscale_factor=2,
                 v_th=0.3, v_reset=0.0, tau=2.0, spike_type='binary', soft_reset=False, 
                  device='cuda'):
        super().__init__()
        # self.device = device
        self.reconstruction_net = StackedFusionReconstructionNet(
